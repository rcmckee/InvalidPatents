...
input_file = open('/Users/robertmckee/Tresors/Projects/InvalidPatentFiles/googleUrlList.txt', 'r')
count_lines = 0
...
#     the above line only worked on lense.org so I had to change the user-agent to access google. http://wolfprojects.altervista.org/changeua.php
    ...
    file_name = '/Users/robertmckee/Tresors/Projects/InvalidPatentFiles/1stFileGoogle/' + str((numpat.strip()) + '_google.html') # I finally prevented the new line between numpat and '_google.html' by putting numpat.strip() in parenthasis (numpat.strip(). )here I couldn't use the website address because the dashes and the comma after numpat prevents the creation of a new line in the file name which doesn't work either
